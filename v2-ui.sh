...
install() {
    bash <(curl -Ls https://ghproxy.com/https://raw.githubusercontent.com/mxfade/v2Ray_ui/master/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
install_bbr() {
    bash <(curl -L -s https://ghproxy.com/https://raw.githubusercontent.com/mxfade/v2Ray_ui/master/bbr.sh)
    if [[ $? == 0 ]]; then
        echo ""
...
update_shell() {
    wget -O /usr/bin/v2-ui -N --no-check-certificate https://hub.fastgit.org/mxfade/v2Ray_ui/raw/master/v2-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
